...
// void  timing(void);
// void  timing(int id_station);
void timing(int id_transfer, int id_station);
// void  event_schedule(float time_of_event, int type_of_event);
// void  event_schedule(int id_station, float time_of_event, int type_of_event);
void event_schedule(int id_transfer, int id_station, float time_of_event, int type_of_event);
...
/* Place transfr into list "list".
   ...
   option = FIRST place at start of list
    ...
    for (item = 0; item <= maxatr; ++item)
        (*row).value[item] = transfer[id_transfer][item];
...
/* Remove a record from list "list" and copy attributes into transfer.
   ...
   option = FIRST remove first record in the list
    ...
    free((char *)transfer[id_transfer]);
    transfer[id_transfer] = (*row).value;
    free((char *)row);
...
/* Remove next event from event list, placing its attributes in transfer.
   ...
   Set next_event_type to this event type, transfer[2]. */
    ...
    list_remove(id_transfer, FIRST, LIST_EVENT + id_station);
...
/* Remove the first event of type event_type from the event list, leaving its
   ...
   if no match is found, event_cancel returns 0. */
    ...
    if(list_size[LIST_EVENT + id_station] == 0) return 0;

    /* Search the event list. */

    row   = head[LIST_EVENT + id_station];
    low   = event_type - EPSILON;
    high  = event_type + EPSILON;
    value = (*row).value[EVENT_TYPE] ;

    while (((value <= low) || (value >= high)) && (row != tail[LIST_EVENT + id_station])) {
        row   = (*row).sr;
        value = (*row).value[EVENT_TYPE];
    }
    ...
    if (row == tail[LIST_EVENT + id_station]) {

        /* Double check to see that this is a match. */

        if ((value > low) && (value < high)) {
            list_remove(id_transfer, LAST, LIST_EVENT + id_station);
            return 1;
        }
    ...
    if (row == head[LIST_EVENT + id_station]) {
        list_remove(id_transfer, FIRST, LIST_EVENT + id_station);
        return 1;
    }
    ...
    list_size[LIST_EVENT + id_station]--;

    /* Copy and free memory. */

    free((char *)transfer[id_transfer]);       /* Free the old transfer. */
    transfer[id_transfer] = (*row).value;      /* Transfer the data. */
...
/* Initialize, update, or report statistics on discrete-time processes:
   ...
   for sampst variable "variable", where "variable":
    ...
    if(variable < 0) { /* Report summary statistics in transfer. */
        ivar        = -variable;
        _transfer[2] = (float) num_observations[ivar];
        _transfer[3] = max[ivar];
        _transfer[4] = min[ivar];
        if(num_observations[ivar] == 0)
            _transfer[1] = 0.0;
        else
            _transfer[1] = sum[ivar] / _transfer[2];
        return _transfer[1];
    }
...
void pprint_out(FILE *unit, int i) /* Write ith entry in transfer to file
                                      "unit". */
{
    if(_transfer[i] == -1e30 || _transfer[i] == 1e30)
        fprintf(unit," %#15.6G ", 0.00);
    else
        fprintf(unit," %#15.6G ", _transfer[i]);
}
