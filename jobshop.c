...
int main(int argc, char **argv)  /* Main function. */
{
   ...
   outfile = fopen(fileOutput, "w");
    ...
    for (id_station = 0, id_transfer = 0; id_station < 1; ++id_station, ++id_transfer) {
                             ...
                             route. */
{
    ...
    if (new_job == 1) {
        ...
        event_schedule(id_station, id_station, sim_time + expon(mean_interarrival, STREAM_INTERARRIVAL),
                       EVENT_ARRIVAL);
    ...
    if (num_machines_busy[station] == num_machines[station]) {
        ...
        transfer[id_station][1] = sim_time;
        transfer[id_station][2] = job_type;
        transfer[id_station][3] = task;
        list_file(id_station, LAST, station);
    }
        ...
        transfer[id_station][3] = job_type;
        transfer[id_station][4] = task;
                      ...
                      station. */
{
    ...
    job_type = transfer[id_station][3];
    task     = transfer[id_station][4];
    station  = route[job_type][task];
    ...
    if (list_size[station] == 0) { // as station.lenght == 0
        ...
        sampst(sim_time - transfer[id_station][1], station);

        /* Tally this same delay for this job type. */

        job_type_queue = transfer[id_station][2];
        task_queue     = transfer[id_station][3];
        sampst(sim_time - transfer[id_station][1], num_stations + job_type_queue);
        ...
        station  = route[job_type_queue][task_queue] - 1;
